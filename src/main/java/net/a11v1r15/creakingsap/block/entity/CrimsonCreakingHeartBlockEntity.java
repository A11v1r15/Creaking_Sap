...
import net.a11v1r15.creakingsap.block.CrimsonCreakingHeartBlock;
...
public class CrimsonCreakingHeartBlockEntity extends AbstractCreakingHeartBlockEntity {
    public CrimsonCreakingHeartBlockEntity(BlockPos pos, BlockState state) {
        super(pos, state);
    }
    ...
    public static void tick(World world, BlockPos pos, BlockState state, AbstractCreakingHeartBlockEntity blockEntity) {
        ...
        if (blockEntity.creakingUpdateTimer-- < 0) {
            ...
            if (blockEntity.creakingPuppet != null) {
                if (CrimsonCreakingHeartBlock.isWorldNaturalAndNight(world) && !(blockEntity.creakingPuppet.squaredDistanceTo(Vec3d.ofBottomCenter(pos)) > 1156.0)) {
                    if (blockEntity.creakingPuppet.isRemoved()) {
                        blockEntity.creakingPuppet = null;
                    }

                    if (!CrimsonCreakingHeartBlock.shouldBeEnabled(state, world, pos) && blockEntity.creakingPuppet == null) {
            ...
            } else if (!CrimsonCreakingHeartBlock.shouldBeEnabled(state, world, pos)) {
                world.setBlockState(pos, state.with(AbstractCreakingHeartBlock.CREAKING, AbstractCreakingHeartBlock.Creaking.DISABLED), 3);
            } else {
                if (!CrimsonCreakingHeartBlock.isWorldNaturalAndNight(world)) {
