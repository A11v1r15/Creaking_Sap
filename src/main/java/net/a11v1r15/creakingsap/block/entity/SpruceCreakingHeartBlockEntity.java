...
import net.a11v1r15.creakingsap.block.SpruceCreakingHeartBlock;
...
public class SpruceCreakingHeartBlockEntity extends AbstractCreakingHeartBlockEntity {
    ...
    public static void tick(World world, BlockPos pos, BlockState state, AbstractCreakingHeartBlockEntity blockEntity) {
        ...
        if (blockEntity.creakingUpdateTimer-- < 0) {
            ...
            if (blockEntity.creakingPuppet != null) {
                if (SpruceCreakingHeartBlock.isWorldNaturalAndNight(world) && !(blockEntity.creakingPuppet.squaredDistanceTo(Vec3d.ofBottomCenter(pos)) > 1156.0)) {
            ...
            } else if (!SpruceCreakingHeartBlock.shouldBeEnabled(state, world, pos)) {
                world.setBlockState(pos, state.with(AbstractCreakingHeartBlock.CREAKING, AbstractCreakingHeartBlock.Creaking.DISABLED), 3);
            } else {
                if (!SpruceCreakingHeartBlock.isWorldNaturalAndNight(world)) {
